...
//        dd($recipes);
    ...
    public function load(Request $request)
    {
        ...
        try {
            ...
            // Chuẩn bị dữ liệu đầu vào cho thuật toán K-means
            $data = [];
            foreach ($recipes as $recipe) {
                $data[] = [$recipe->level, $recipe->id_category];
            }

            // Chạy thuật toán K-means trên ma trận dữ liệu đầu vào
            $kmeans = new KMeans(2); // chia nhóm
            $clusters = $kmeans->cluster($data);

            // Lấy ra các công thức trong cùng một nhóm với công thức được yêu cầu
            $recipe = Recipes::where('id', $request->id)->first();
            $searchKey = [$recipe->level, $recipe->id_category];
            $cluster_key = array_keys($clusters, $clusters[array_search($searchKey, $data)]);

            $related_recipes = [];
            foreach ($cluster_key as $key) {
                $related_recipes[] = $recipes[$key];
            }
